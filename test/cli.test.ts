...
function createLoadedConfig(tags?: string[]) {
  return {
    config: {
      ...
      cursor: { type: "dot", size: 12, borderWidth: 2, borderColor: "#fff", shadowColor: "#000", start: { x: 0, y: 0 }, persistPosition: true },
      motion: { moveDurationMs: 600, moveStepsMin: 25, stepsPerPx: 12, clickDelayMs: 60, curve: { offsetRatio: 0.1, offsetMin: 4, offsetMax: 80, easing: "easeInOutCubic" } },
...
describe("cli", () => {
  ...
  it("parses tags, output dir, and script hints", async () => {
    process.argv = ["node", "cli", "demo", "--tag=marketing, onboarding", "--tag", "sales", "-o", "./videos", "--hint", "one", "--hint", "two"];
  ...
  it("routes script generate subcommand with description, url, and hints", async () => {
    process.argv = ["node", "cli", "script", "generate", "Show signup", "--url", "https://app.example.com", "--name", "signup", "--hint", "Use hero"];

    const { runCli } = await import("../src/cli.js");

    await expect(runCli()).resolves.toBe(0);
    expect(scriptGenerate).toHaveBeenCalledWith("Show signup", "https://app.example.com", "signup", {
      verbose: false,
      headed: undefined,
      noCache: undefined,
      hints: ["Use hero"],
  ...
  it("routes full script pipeline when the script command is given a description", async () => {
    process.argv = ["node", "cli", "script", "Show signup", "--url", "https://app.example.com", "--voice", "nova", "--speed", "1.25", "--resolution", "2K", "--format", "webm"];
  ...
  it("accepts a direct scenario file path when it exists", async () => {
    ...
    expect(loadConfig).toHaveBeenCalledWith(expect.stringContaining("/workspace/project/custom/demo.demo.ts"), "./videos");
    expect(runVideoScenario).toHaveBeenCalledTimes(1);
  });
