...
function formatPage(page: PageInfo): string {
	...
	const inputs = page.elements.filter((el) =>
	...
	const navLinks = page.elements.filter(
		(el) => el.tag === "a" && el.href?.startsWith("/"),
	);
...
async function main() {
	...
	const newTemplateLink = landing.elements.find(
		(el) => el.href?.includes("/templates/new"),
	);
