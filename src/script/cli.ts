...
export async function scriptVoice(
  ...
  const { timedScenes, narrationManifestPath } = await generateNarrationAudio(segments, audioPath, {
    verbose,
  });
...
export async function scriptBuild(
  ...
  if (!script.audioPath) {
    throw new Error("Script has no audio timing data. Run `demo-reel script voice` first.");
  }
...
export async function scriptValidate(
  ...
  for (const f of failures) {
    ...
    console.log(
      `  Scene ${f.scene + 1}, Step ${f.step + 1} (${(scene.steps[f.step] as { action: string }).action}): ${f.error}`,
    );
  }
 ...
 */
export async function scriptFix(scriptPath: string, options: ScriptCliOptions = {}): Promise<void> {
  const { verbose, headed } = options;
