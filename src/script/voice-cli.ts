...
function pickVoiceOverrides(source: unknown): VoiceConfigOverridesSource {
	...
	return {
		provider: value.provider === "piper" || value.provider === "openai" || value.provider === "elevenlabs"
			? value.provider
			: undefined,
...
async function main() {
	...
	try {
		...
		const voice = resolveVoiceConfig({
			provider: cliProvider as VoiceConfig["provider"] | undefined ?? scriptVoice.provider ?? productConfig.provider,
		...
		const { timedScenes, narrationManifestPath } = await generateNarrationAudio(segments, audioPath, { verbose: true });
